...
class Exp(BaseExp):
    def __init__(self):
        ...
        self.swin_drop_path_rate = 0.2
        self.swin_pretrained = True
        self.swin_pretrained_type = "COCO" #"COCO" or "ImageNet"
        self.swin_pretrained_checkpoint = "./pretrained/swin_pretrained/mask_rcnn_swin_tiny_patch4_window7.pth"
    ...
    def get_model(self):
        ...
        if getattr(self, "model", None) is None:
            #in_channels = [192, 384, 768] # 256, 512, 768
            in_channels = self.swin_out_channels[1:]
            backbone = YOLOPAFPN(
                           depth = self.depth,
                           width = self.width,
                           ...
                           swin_drop_path_rate = self.swin_drop_path_rate,
                           swin_pretrained_type = self.swin_pretrained_type,
                           swin_pretrained_checkpoint = self.swin_pretrained_checkpoint, 
                           act = self.act,
                       )
